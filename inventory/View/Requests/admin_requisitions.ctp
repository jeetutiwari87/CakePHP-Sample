...
<aside class="requests right-side">
  <section class="content-header">
    <h1> <?php echo __('View Requisitions'); ?> <small><?php echo __('Here you can manage all View Requisitions');?></small> </h1>
...
foreach ($requests as $request): ?>
                    <tr>
                      ...
                      <td class="actions\ center"><?php if ($permission_obj->CheckPermission($UsersDetails['role_id'], 'admin_requisition', 'is_read')) { ?>
                        <?php echo $this->Html->link('<i class="fa fa-pause"></i>'.__('View'), array('action' => 'details', $request['Request']['id']),array('escape' => false,'class'=>'btn btn-app view')); ?>
                        <?php } ?>
						 <?php if ($request['Request']['request_status'] == '1'): ?>
                        <?php if ($permission_obj->CheckPermission($UsersDetails['role_id'], 'admin_requisition', 'is_edit')) { ?>
                        <?php echo $this->Html->link('<i class="fa fa-edit"></i>'.__('Collect'), array('action' => 'collect', $request['Request']['id']), array('escape' => false,'class'=>'btn btn-app edit')); ?>
                        <?php } ?>
						 <?php endif; ?>
						 
						 <?php if ($request['Request']['request_status'] >3): ?>
                        <?php if ($permission_obj->CheckPermission($UsersDetails['role_id'], 'admin_requisition', 'is_edit')) { ?>
                        <?php echo $this->Html->link('<i class="fa fa-edit"></i>'.__('Verify'), array('action' => 'verify', $request['Request']['id']), array('escape' => false,'class'=>'btn btn-app edit')); ?>
                        <?php } ?>
						 <?php endif; ?>
						 
						  <?php if ($permission_obj->CheckPermission($UsersDetails['role_id'], 'admin_requisition', 'is_delete')) { ?>
