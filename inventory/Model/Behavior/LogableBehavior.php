...
class LogableBehavior extends ModelBehavior {
	...
	public function afterDelete(Model $Model) {
		...
		if ($this->Log->hasField('description')) {
		    if($Model->alias=='RemedialWork'){
			 $logData['Log']['description'] = 'Remedial project ';
			}elseif($Model->alias=='PropertyAsbestoSurvey'){
			 $logData['Log']['description'] = 'Property Asbestos Survey ';
			}elseif($Model->alias=='PropertyAsbesto'){
			 $logData['Log']['description'] = 'Asbestos Item ';
			}elseif($Model->alias=='PropertyNote'){
			 $logData['Log']['description'] = 'Property Note ';
			}elseif($Model->alias=='AsbestosFibreType'){
			 $logData['Log']['description'] = 'Asbestos Fibre Type ';
			}elseif($Model->alias=='PropertyRoom'){
			 $logData['Log']['description'] = 'Property Room ';
			}elseif($Model->alias=='User' && $this->user['User']['role_id']=='3'){
			 $logData['Log']['description'] = 'Client ';
			}elseif($Model->alias=='User' && $this->user['User']['role_id']=='2'){
			 $logData['Log']['description'] = 'Consultant ';
			}else{
	...
	public function _saveLog(Model $Model, $logData, $title = null) {
		...
		if ($this->Log->hasField($this->settings[$Model->alias]['userKey']) && $this->user && isset($this->user['User'])) {
			$logData['Log']['user_id'] = $this->user['User'][$this->UserModel->primaryKey];
		}
		...
		if ($this->Log->hasField('description')) {
			...
			if ($this->user && $this->UserModel && isset($this->user['User'])) {
				$logData['Log']['description'] .= ' ' . __('by') . ' "' . $this->user['User']['username'] . '"'; //' . $this->settings[$Model->alias]['userModel'] . '
				/*if ($this->settings[$Model->alias]['descriptionIds']) {
					$logData['Log']['description'] .= ' (' . $this->user['User']['username'] . ')';
				}*/
