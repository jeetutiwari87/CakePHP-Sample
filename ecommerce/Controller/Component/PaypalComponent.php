...
class PaypalComponent extends Object
{
    ...
    public function sanitizeServerVars($POST) 
    {
        
        $this->paypal_post_arr = (!empty($POST)) ? $POST : array();
    ...
    function doDirectPayment($post_info, $sender_info) 
    {
        ...
        $expDateMonth = urlencode($post_info['expDateMonth']);
        // Month must be padded with leading zero
        $padDateMonth = str_pad($expDateMonth, 2, '0', STR_PAD_LEFT);
		
        $expDateYear = urlencode($post_info['expDateYear']);
        ...
        $currencyCode = 'USD'; //only USD allowed
        $paymentType = urlencode($post_info['paymentType']);
